...
transaction_detail = TransactionViewSet.as_view({"get": "retrieve", "put": "update"})
    ...
    def get_queryset(self):
        ...
        return (
            Transaction.objects.filter(
                user=self.request.user, type="expense", account__type=2, **qp
            )
            .exclude(category__group__type="neutral")
            .values("category__group__name")
...
class GroupedIncomeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            Transaction.objects.filter(
                user=self.request.user, type="income", account__type=2, **qp
            )
            .exclude(category__group__type="neutral")
            .values("category__group__name")
            .annotate(total=Sum("amount"))
...
class AverageExpensesViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return [
            ...
            .exclude(category__group__type="neutral")
            .filter(date__gte=firstInitMonth)
            .filter(date__lte=firstLastMonth)
            .annotate(month=TruncMonth("date"))
            .values("month")
            .annotate(monthly_total=Sum("amount"))
            .aggregate(total=Avg("monthly_total"))
        ]
...
class MonthwiseExpensesViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return [
            Transaction.objects.filter(
                user=self.request.user, type="expense", account__type=2
            )
            .exclude(category__group__type="neutral")
            .filter(date__gte=firstInitMonth)
            .filter(date__lte=firstLastMonth)
...
class MonthwiseIncomeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return [
            Transaction.objects.filter(
                user=self.request.user, type="income", account__type=2
            )
            .exclude(category__group__type="neutral")
            .filter(date__gte=firstInitMonth)
            .filter(date__lte=firstLastMonth)
...
class AverageIncomeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return [
            ...
            .exclude(category__group__type="neutral")
            .filter(date__gte=datetime.strptime("2020-02-01", "%Y-%m-%d"))
            .annotate(month=TruncMonth("date"))
            .values("month")
            .annotate(monthly_total=Sum("amount"))
            .aggregate(total=Avg("monthly_total"))
...
class CompleteMonthlyIncomeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qp = {k: v for k, v in self.request.query_params.items()}
        return [
            Transaction.objects.filter(user=self.request.user, type="income", **qp)
            .exclude(category__group__type="neutral",)
            .values("type")
            .aggregate(total=Sum("amount"))
        ]
...
class TransactionFilteredView(generics.GenericAPIView, mixins.ListModelMixin):
    serializer_class = serializers.TransactionSerializer

    def get_queryset(self):
        qp = {k: v for k, v in self.request.query_params.items()}
        return (
            Transaction.objects.filter(user=self.request.user, account__type=2, **qp)
            .exclude(category__group__type="neutral")
            .order_by("-amount")
        )
...
class CurrentAccountValuesView(generics.GenericAPIView, mixins.ListModelMixin):
    serializer_class = serializers.AccountValueSerializer

    def get_queryset(self):
        acc_vals = (
            AccountValue.objects.filter(user=self.request.user)
    ...
    def account_values_to_target_currency(self, acc_vals, target_currency_code="CLP"):
        rates = (
            ExchangeRate.objects.filter(target__code=target_currency_code)
        ...
        for acc_val in acc_vals:
            ...
            if not cur_currency_code == target_currency_code:
                rate = [
                    elem.rate for elem in rates if elem.origin.code == cur_currency_code
                ][0]
...
class NetWorthView(generics.GenericAPIView, mixins.ListModelMixin):
    serializer_class = serializers.AggregateSerializer

    def get_queryset(self):
        nw = (
            NetWorth.objects.filter(user=self.request.user, type="networth")
            .order_by("-valued_at")[0]
            .value
        )
        return [{"total": nw}]
...
class NetWorthHistoryView(generics.GenericAPIView, mixins.ListModelMixin):
    serializer_class = serializers.NetworthSerializer

    def get_queryset(self):
        return NetWorth.objects.filter(
            user=self.request.user, type="networth"
        ).order_by("valued_at")
...
class RetirementHistoryView(generics.GenericAPIView, mixins.ListModelMixin):
    serializer_class = serializers.NetworthSerializer

    def get_queryset(self):
        return NetWorth.objects.filter(
            user=self.request.user, type="retirement"
        ).order_by("valued_at")
...
class RetirementInvestmentHistoryView(generics.GenericAPIView, mixins.ListModelMixin):
    serializer_class = serializers.NetworthSerializer

    def get_queryset(self):
        return NetWorth.objects.filter(
            user=self.request.user, type="retirement_investment"
        ).order_by("valued_at")
...
class SavingsHistoryView(generics.GenericAPIView, mixins.ListModelMixin):
    serializer_class = serializers.NetworthSerializer

    def get_queryset(self):
        return NetWorth.objects.filter(user=self.request.user, type="savings").order_by(
            "valued_at"
        )
...
class SavingsInvestmentHistoryView(generics.GenericAPIView, mixins.ListModelMixin):
    serializer_class = serializers.NetworthSerializer

    def get_queryset(self):
        return NetWorth.objects.filter(
            user=self.request.user, type="savings_investment"
        ).order_by("valued_at")
...
class CurrentAssetValuesView(generics.GenericAPIView, mixins.ListModelMixin):
    serializer_class = serializers.AssetValueSerializer

    def get_queryset(self):
        asset_vals = (
            AssetValue.objects.filter(user=self.request.user)
            .order_by("asset__name", "-valued_at")
            .distinct("asset__name")
        )
        return self.asset_values_to_target_currency(asset_vals)

    def asset_values_to_target_currency(self, asset_vals, target_currency_code="UF"):
        rates = (
            ExchangeRate.objects.filter(target__code=target_currency_code)
            .order_by("origin__code", "target__code", "-valued_at")
            .distinct("origin__code", "target__code")
