...
def collect_rate_currency(cur):
    ...
    response = requests.get(
        "https://transferwise.com/gb/currency-converter/{}-to-clp-rate".format(
            cur.lower()
        )
    )
...
def collect_value_fondo(fondo):
    payload = {"tf": fondo}
    response = requests.get(
        "https://www.spensiones.cl/apps/valoresCuotaFondo/vcfAFP.php", params=payload
    )
...
def post_to_db(value, origin):
    ...
    models.ExchangeRate.objects.create(
        origin_id=origin, target_id=1, rate=value, valued_at=datetime.now()
    )
...
def extract_value_uf(response):
    p = re.compile(
        '<td class="text-right">1</td><td class="text-right">\d{1,3}.\d{1,4},\d{1,3}</td>'
    )
