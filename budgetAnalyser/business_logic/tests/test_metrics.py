...
class GetSavingAccountValues(TestCase):
    def setUp(self):
        ...
        self.account_type = AccountType.objects.create(type="NORMAL")
        self.account = Account.objects.create(
            name="test-account", bank=self.bank, user=self.user, type=self.account_type
        )
    ...
    def test_returns_the_closest_value_when_provided_a_when(self):
        ...
        res = metrics.get_savings_account_values(
            self.user, when=pytz.utc.localize(datetime(2020, 7, 2, 14, 30, 0)),
        )
    ...
    def test_returns_empty_qset_value_when__no_values_exists(self):
        self.client.login(email="user@gmail.com", password="abc")
        res = metrics.get_savings_account_values(
            self.user, when=datetime(2010, 7, 2, 14, 30, 0),
        )
        assert len(res) == 0
...
class TotalSavingTestCase(TestCase):
    ...
    def test_returns_the_closest_total_in_correct_currency_when_a_date_is_provided(
        self,
    ):
        self.client.login(email="user@gmail.com", password="abc")
        res = metrics.savings(
